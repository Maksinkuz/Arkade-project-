import arcade
import math
import time
from constants import *


class Entity(arcade.Sprite):
    """ Базовый класс для всех сущностей """

    def __init__(self, filename, scale):
        super().__init__(filename, scale=scale)


class Player(Entity):
    def __init__(self):
        # Используем анимацию
        super().__init__(":resources:images/animated_characters/female_person/femalePerson_idle.png", PLAYER_SCALE)
        self.hp = 1000
        self.has_key = False
        self.idle_texture = arcade.load_texture(RESOURCE_PLAYER)
        self.walk_textures = [arcade.load_texture(RESOURCE_PLAYER_WALK[i]) for i in range(7)]
        self.counter = 0
        self.walk_texture = self.walk_textures[0]
        self.texture = self.idle_texture

    def update(self, delta_time):
        print(delta_time)
        self.center_x += self.change_x * delta_time
        self.center_y += self.change_y * delta_time
        if delta_time > 0.007:
            self.counter = (self.counter + 1) % len(RESOURCE_PLAYER_WALK)
            self.walk_texture = self.walk_textures[self.counter]

        if abs(self.change_x) > 0 or abs(self.change_y) > 0:
            self.texture = self.walk_texture
        else:
            self.texture = self.idle_texture

        if self.change_x < 0:
            self.texture = self.walk_texture.flip_left_right()
        elif self.change_x > 0:
            self.texture = self.walk_texture



class AcidPuddle(arcade.Sprite):

    def __init__(self, x, y, size=20):
        super().__init__(scale=1.0)
        self.texture = arcade.make_circle_texture(size, arcade.color.BRIGHT_GREEN)
        self.center_x = x
        self.center_y = y
        self.creation_time = time.time()

    def is_expired(self):
        return time.time() - self.creation_time > PUDDLE_LIFE_TIME


class EnemyBullet(arcade.Sprite):

    def __init__(self, start_x, start_y, target_x, target_y):
        super().__init__(scale=1.0)
        self.texture = arcade.make_circle_texture(7, arcade.color.BRIGHT_GREEN)
        self.center_x = start_x
        self.center_y = start_y

        # Расчет вектора движения
        x_diff = target_x - start_x
        y_diff = target_y - start_y
        angle = math.atan2(y_diff, x_diff)

        self.change_x = math.cos(angle) * BULLET_SPEED
        self.change_y = math.sin(angle) * BULLET_SPEED


class Enemy(Entity):


    def __init__(self):
        super().__init__(":resources:images/animated_characters/zombie/zombie_idle.png", ENEMY_SCALE)
        self.last_shot_time = 0
        self.hp = 30  # Здоровье зомби
        self.idle_texture = arcade.load_texture(RESOURCE_ENEMY)
        self.walk_textures = [arcade.load_texture(RESOURCE_ENEMY_WALK[i]) for i in range(7)]
        self.counter = 0
        self.walk_texture = self.walk_textures[0]
        self.texture = self.idle_texture

    def follow_player(self, player_sprite, wall_list):


        # 1. Расчет дистанции
        distance = arcade.get_distance_between_sprites(self, player_sprite)

        # 2. Проверка видимости
        has_los = arcade.has_line_of_sight(self.position, player_sprite.position, wall_list)

        if distance < ENEMY_SIGHT_DISTANCE and has_los:
            # Движение к игроку

            x_diff = player_sprite.center_x - self.center_x
            y_diff = player_sprite.center_y - self.center_y
            angle = math.atan2(y_diff, x_diff)

            # Идем к игроку, но не вплотную
            self.center_x += math.cos(angle) * ENEMY_SPEED
            self.center_y += math.sin(angle) * ENEMY_SPEED

            # Разворот спрайта
            if x_diff < 0:
                self.face_direction = "left"

            # Стрельба
            if time.time() - self.last_shot_time > ENEMY_ATTACK_COOLDOWN:
                self.last_shot_time = time.time()
                return True

        return False

